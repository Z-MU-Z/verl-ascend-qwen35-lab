...
- After patching `sleep()` to skip that path (`7d312155`), the next `.36` rerun `/home/zmz/verl/log_archive/qwen35_4b_freezevis_singlecard_nosleepskip_20260413_101327.log` progressed further into rollout resume, but then failed in `wake_up()` with the same allocator assertion.
- The later archived run `/home/zmz/verl/log_archive/qwen35_4b_freezevis_singlecard_nowakeskip_20260413_104643.log` showed the tighter root cause: `resume(tags=["weights"]) -> ServerAdapter._execute_method(...) -> vLLMHttpServer.collective_rpc("wake_up") -> vllm_ascend/worker/worker.py -> CaMemAllocator`. In other words, guarding the wrapper `wake_up()` method alone was not enough.
- On this fallback line, `enable_sleep_mode=False` must therefore imply skipping both wrapper-level and adapter-level sleep/wake calls; otherwise `collective_rpc("wake_up")` can still re-enter the Ascend memory-pool path.
- In the required `/home/zmz/envs/qwen35-t29-lite` env on `.36`, the latest archived run `/home/zmz/verl/log_archive/qwen35_4b_freezevis_t29lite_n8_20260413_133024.log` advanced past the earlier sleep/wake and custom-op-import blockers, then failed deeper in `EngineCore` because `torch.ops._C_ascend.npu_causal_conv1d_custom` is absent in that env. Keep this separate from the older shared-env failures when reporting status.
- The local repo now carries a Python fallback for missing `_C_ascend.npu_causal_conv1d_custom`, wired in the same NPU patch path as the earlier `npu_gemma_rms_norm` workaround. Sync that repo change first, then rerun before concluding that the active `t29-lite` env is fundamentally blocked.
- The first version of that fallback, archived in `/home/zmz/verl/log_archive/qwen35_4b_freezevis_t29lite_n8_causalconvfix_20260413_164416.log`, proved that the missing-op blocker was bypassed, but then failed later with `AttributeError: 'function' object has no attribute 'scalar'` inside the causal-conv execution path. Treat that as an intermediate failed workaround, not as the final state.
- The newer reference-path fallback, archived in `/home/zmz/verl/log_archive/qwen35_4b_freezevis_t29lite_n8_refconvfix_20260413_173332.log`, advanced beyond startup and into actual training: initial validation completed, `Training Progress` advanced, and `global_step_5` checkpoint files were written under `/home/zmz/verl/ckpts/GRPO-Qwen3_5/GRPO-Qwen3_5-4B/`. This is the current best-known `.36` path.
